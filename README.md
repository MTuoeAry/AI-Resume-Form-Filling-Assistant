...
- 标准简历 Schema v7：覆盖拼音姓名、证件与地址、教育、实习、项目、结构化奖项/专利/论文、语言及长尾自定义字段，并兼容旧版汇总字段迁移
- 校招经历分流：正式工作经历允许保持 0 条；实习经历不会映射到 `workExperiences`，旧数据中明确标注的实习条目会迁移到 `internships`
- AI 导入简历：支持粘贴原始简历文本，或者上传带文字层的 PDF，自动抽取并整理为标准简历 JSON
- 三种填充模式：
  - 整页填充：覆盖当前页面可识别字段
  - 增量填入：跳过已经有值的字段
  - 选区填入：先框选页面区域，只填选区内字段
- 多控件支持：`input`、`textarea`、`select`、`radio group`、`checkbox group`、`contenteditable`、搜索型自定义下拉和部分级联选择器
- 日期控件适配：对部分只读日期输入框和日期面板做了特殊处理
- 复合字段识别：为起止时间、手机号区号/号码、证件类型/号码标注角色，避免相邻控件重复映射
- 受控新增经历：只依据标准简历里真实存在的条目数，精确匹配对应区块的“新增经历”入口，并逐次确认页面已增加字段
- 本机附件管理：可预先配置个人照片、简历附件和相关作品，匹配到文件控件后本地选择文件
...
- 便携简历文档：可导出 `.airesume.json`，在其他电脑合并导入或完全替换；不包含附件、模型配置和 API Key
...
如果标准简历里存在多条教育、实习、项目、奖项、专利、论文或语言记录，扩展会先在对应页面区块中准备足够数量的空白条目。点击目标必须与站点规则或保守的通用规则精确匹配；每次点击后都要确认控件数量确实增长，并受单次总量上限保护。
...
- `导出文档`：下载 `ai-resume-profile-YYYYMMDD.airesume.json`
- `合并非空字段`：导入文档中的非空值覆盖同名字段，空值不会擦除当前数据
- `完全替换`：以导入文档为准，适合在新电脑恢复完整简历

可以把该文件保存在私有同步盘、移动硬盘或私有仓库。附件需单独在每台电脑上配置。

你也可以维护多份简历模板（例如分别面向测开、游戏开发、普通开发）。在侧边栏或简历配置页里切换、新建、复制、重命名、删除模板；填充前在“自动填充”页选择要使用的那一份。所有模板可一键导出为 JSON 备份，也可从 JSON 恢复。全部模板备份包含各模板的原始简历文本，但不包含附件、模型配置或 API Key。
...
- 搜索型自定义下拉与部分级联选择器
- 部分只读日期输入框与日期面板
- 个人照片、简历附件和相关作品的本机预配置与文件控件写入
- 整页/增量/选区三种填充模式

### 明确限制

- 网站要求在线裁剪、验证码或专用上传协议时仍需人工完成后续步骤
...
│   ├── profile-document.js     # .airesume.json 导入、导出、校验与非空合并
│   ├── asset-storage.js        # 照片、简历附件和作品的 IndexedDB 本机存储
│   ├── site-adapters.js        # 保守通用规则与轻量站点能力配置
...
- 预配置附件存放在扩展自己的 IndexedDB 中；附件内容不会发给字段映射模型，只会在你点击填充时交给当前目标页面的文件控件
