(function initResumeStorage(root) {
  ...
  const keys = Object.freeze({
    ...
    // 旧版单模板 key。当前活动模板会持续镜像到这些 key，便于版本回退。
  ...
  const MAX_IMPORTED_TEMPLATES = 50;
  ...
  function persistTemplates(storage, templates, activeTemplateId) {
    ...
    return storage.local.set({
      ...
      [keys.profile]: active?.profile || {},
      [keys.schemaVersion]: active?.schemaVersion,
      [keys.rawText]: active?.rawText || "",
    });
  }
  ...
  async function loadTemplateState(storageOverride) {
    ...
    // 模板数据损坏或为空时，优先从兼容快照恢复，避免用空模板覆盖旧简历。
    if (templates.length === 0) {
      const legacy = await loadLegacyResumeData(storage);
      const recovered = buildEmptyTemplate(
        DEFAULT_TEMPLATE_ID,
        DEFAULT_TEMPLATE_NAME
      );
      recovered.profile = legacy.profile;
      recovered.rawText = legacy.rawText;
      recovered.schemaVersion = legacy.schemaVersion;
      templates = [recovered];
      activeTemplateId = DEFAULT_TEMPLATE_ID;
      await persistTemplates(storage, templates, activeTemplateId);
    }
    ...
    if (!templates.some((template) => template.id === activeTemplateId)) {
      ...
      // 为已经使用上游多模板格式的用户补建可回退的活动模板快照。
      await persistTemplates(storage, templates, activeTemplateId);
    }

    // 仅清理更早期的废弃 key；当前版单模板 key 保留为回退兼容快照。
    await removeIfAvailable(storage.local, legacyOldKeys);
  ...
  async function setActiveTemplateId(id, storageOverride) {
    ...
    await persistTemplates(storage, state.templates, targetId);
    return targetId;
  }
  ...
  async function importTemplateData(data, storageOverride) {
    ...
    if (data.kind && data.kind !== "resume-templates") {
      throw new Error("导入文件不是简历模板备份");
    }
    if (
      data.version != null &&
      Number(data.version) > TEMPLATE_BACKUP_VERSION
    ) {
      throw new Error(`暂不支持模板备份版本 v${data.version}`);
    }
    ...
    if (rawTemplates.length > MAX_IMPORTED_TEMPLATES) {
      throw new Error(`一次最多导入 ${MAX_IMPORTED_TEMPLATES} 个简历模板`);
    }
